...
class NetworkModel:
    ...
    def loop_trajectories(
        ...
        while (n_iter < n_loops) or ((n_iter >= n_loops) and count_end < count_max):
            ...
            # --- Intermediate save every save_every iterations ---
            if save_path is not None and (n_iter % save_every == 0):
                import numpy as _np
                _np.save(save_path + '_inter_checkpoint', inter)
                _np.save(save_path + '_basal_checkpoint', basal)
                _np.save(save_path + '_inter_tmp_checkpoint', inter_tmp)
                _np.save(save_path + '_basal_tmp_checkpoint', basal_tmp)
                if verb:
                    print(f"[fit_network] Checkpoint saved at iteration {n_iter}")
