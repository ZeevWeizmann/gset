...
from grn_generator import (generate_grn, generate_shared_targets_grn,
                           generate_parallel_sync_grn)
from harissa_optimizer import (HarissaParameterOptimizer, make_scenario,
                               make_shared_targets_scenario,
                               make_parallel_sync_scenario,
                               make_pulse_scenario)
from diagnostics import plot_umap, plot_marginals, load_grn_from_dir
...
TIMEPOINTS = {
    ...
    "switch":         [0, 12, 24, 48, 72, 96, 120, 144, 180, 240],
    "cascade":        [0, 12, 24, 48, 72, 96, 120, 144, 180, 240, 300],
    "cascade_short":  [0, 12, 24, 48, 72, 96, 120, 144],
    "switch_cascade": [0, 12, 24, 48, 72, 96, 120, 144],
    "shared_targets": [0, 12, 24, 48, 72, 96],
    "parallel_sync":  [0, 12, 24, 48, 72, 96, 120, 144, 180, 240],
    "early_response":          [0, 12, 24, 48, 72, 96, 120, 144, 180, 240, 300],
}

# Default optimiser settings per scenario (tuned for convergence speed)
SCENARIO_OPT = {
    "activation":     dict(T_cascade=200.0, n_steps_cascade=600),
    "switch":         dict(T_cascade=300.0, n_steps_cascade=900, cascade_weight=3.0),
    "cascade":        dict(T_cascade=350.0, n_steps_cascade=1000, cascade_weight=3.0),
    "cascade_short":  dict(T_cascade=250.0, n_steps_cascade=750,  cascade_weight=3.0),
    "switch_cascade": dict(T_cascade=250.0, n_steps_cascade=750, cascade_weight=4.0),
    "shared_targets": dict(T_cascade=350.0, n_steps_cascade=1000, special_weight=5.0),
    "parallel_sync":  dict(T_cascade=200.0, n_steps_cascade=600, special_weight=3.0),
    "early_response":          dict(T_cascade=350.0, n_steps_cascade=1000, cascade_weight=4.0),
}
...
def plot_ode_trajectory(optimizer, result, out_path):
    ...
    Special genes (shared targets / hubs) shown with distinct dashed style.
    """
    times, traj = optimizer.simulate_from_resting(result, T=500.0, n_save=250)
    ...
    for k in range(K):
        for i, g in enumerate(optimizer.grn.genes_per_cluster[k]):
            ...
            ax.plot(times, traj[:, g + 1], color=colors[k], alpha=0.6,
                    lw=1.5, label=lbl)

    # Special genes overlay
    for kind in ("shared", "hubs"):
        ids = optimizer.grn.special_ids.get(kind, [])
        for i, g in enumerate(ids):
            lbl = kind.capitalize() if i == 0 else None
            ax.plot(times, traj[:, g + 1], color=special_color, alpha=0.85,
                    lw=2.0, ls="--", label=lbl)
...
def run_pipeline(args):
    ...
    # [1] GRN ─────────────────────────────────────────────────────────────────
    print("\n[1] Generating GRN...")
    if args.scenario == "shared_targets":
        grn = generate_shared_targets_grn(
            n_genes_per_cluster=args.n_genes // max(args.n_clusters, 1),
            n_clusters=args.n_clusters,
            n_tfs_per_cluster=args.n_tfs_per_cluster,
            n_shared=args.n_shared,
            rho_in=args.rho_in, seed=args.seed,
        )
    elif args.scenario == "parallel_sync":
        grn = generate_parallel_sync_grn(
            n_genes_per_cluster=args.n_genes // max(args.n_clusters, 1),
            n_clusters=args.n_clusters,
            n_tfs_per_cluster=args.n_tfs_per_cluster,
            n_hubs=args.n_hubs,
    ...
    elif args.scenario == "early_response":
    ...
    if args.scenario == "shared_targets":
        grn, signs, cell_states, transitions, kp = make_shared_targets_scenario(
            grn, seed=args.seed)
    elif args.scenario == "parallel_sync":
        grn, signs, cell_states, transitions, kp = make_parallel_sync_scenario(
            grn, seed=args.seed)
    elif args.scenario == "early_response":
        grn, signs, cell_states, transitions, kp = make_pulse_scenario(
            grn, seed=args.seed)
    else:
    ...
    # Save special gene indices (ground truth for GSET evaluation)
    if grn.special_ids:
        import json as _json
        with open(out_dir / "special_ids.json", "w") as _f:
            _json.dump({k: [int(v) for v in vs]
                        for k, vs in grn.special_ids.items()}, _f, indent=2)
        print(f"  Special genes: {grn.special_ids}")
    ...
    if args.scenario == "switch":
        # Two-phase simulation: early TPs with stimulus→C0, late TPs with stimulus→C1
        # This ensures cells show C0-active dynamics early and C1-active dynamics late.
        half = len(timepoints) // 2
        tps_early = timepoints[:half]
        tps_late  = timepoints[half:]

        import copy
        result_A = copy.deepcopy(result)
        result_B = copy.deepcopy(result)

        # Stimulus row: wire to C0 TFs only (result_A) or C1 TFs only (result_B)
        c0_tfs = [g for g in range(grn.n_genes) if grn.cluster_ids[g] == 0 and grn.tf_mask[g]]
        c1_tfs = [g for g in range(grn.n_genes) if grn.cluster_ids[g] == 1 and grn.tf_mask[g]]
        stim_A = abs(result["inter"][0, c0_tfs[0] + 1]) if len(c0_tfs) else 3.5
        # Use strong fixed stim for Phase B — C1 TFs have more negative basal and
        # need a much stronger signal to activate than the basal-ratio heuristic provides.
        stim_B = max(stim_A * 3.0, 8.0)

        result_A["inter"][0, :] = 0.0
        for g in c0_tfs:
            result_A["inter"][0, g + 1] = stim_A

        result_B["inter"][0, :] = 0.0
        for g in c1_tfs:
            result_B["inter"][0, g + 1] = stim_B

        print(f"  [switch] Phase A (C0 active): t={tps_early}")
        datasets_A = simulate_multiple_runs(
            G=grn.n_genes, result=result_A,
            scenario=SimulationScenario(timepoints=tps_early,
                                        n_cells_per_tp=args.n_cells_per_tp,
                                        burnin=args.burnin),
            n_runs=args.n_runs, seed=args.seed, verbose=True, n_jobs=args.n_jobs)

        print(f"  [switch] Phase B (C1 active): t={tps_late}")
        datasets_B = simulate_multiple_runs(
            G=grn.n_genes, result=result_B,
            scenario=SimulationScenario(timepoints=tps_late,
                                        n_cells_per_tp=args.n_cells_per_tp,
                                        burnin=args.burnin),
            n_runs=args.n_runs, seed=args.seed + 1, verbose=True, n_jobs=args.n_jobs)

        # Combine: concatenate rows (skip header of B datasets)
        datasets = []
        for dA, dB in zip(datasets_A, datasets_B):
            combined = np.vstack([dA, dB[1:]])
            datasets.append(combined)
    elif args.scenario == "early_response":
        # True pulse: C0 activates (onset) then returns to resting (offset).
        # Phase A: stimulus→C0 on  → C0 rises (early TPs)
        # Phase B: stimulus off     → cells relax to resting (late TPs)
        # This creates a non-monotonic C0 trajectory that challenges gene_ot:
        # OT sees C0-high at mid-TPs coupling to C0-low at late-TPs and loses direction.
        half = len(timepoints) // 2
        tps_early = timepoints[:half]   # onset: resting → C0 peak
        tps_late  = timepoints[half:]   # offset: C0 decays → resting
        ...
        # Phase A: stimulus drives C0
        result_A["inter"][0, :] = 0.0
        for g in c0_tfs:
            result_A["inter"][0, g + 1] = stim_A

        # Phase B: no stimulus — cells relax to resting
        result_B["inter"][0, :] = 0.0

        print(f"  [pulse] Phase A (C0 onset, stimulus on): t={tps_early}")
        datasets_A = simulate_multiple_runs(
            G=grn.n_genes, result=result_A,
            scenario=SimulationScenario(timepoints=tps_early,
                                        n_cells_per_tp=args.n_cells_per_tp,
        ...
        print(f"  [pulse] Phase B (C0 offset, stimulus off → resting): t={tps_late}")
        datasets_B = simulate_multiple_runs(
            G=grn.n_genes, result=result_B,
            scenario=SimulationScenario(timepoints=tps_late,
                                        n_cells_per_tp=args.n_cells_per_tp,
    ...
    # Reload grn from saved files (guarantees consistency with data)
    grn_saved = load_grn_from_dir(str(out_dir))
    data_path0 = str(data_dir / "data_1.txt")
    if not args.skip_umap:
        plot_umap(data_path0, grn_saved, out_dir / "umap.png",
                  extra_title=args.scenario)
    plot_marginals(data_path0, grn_saved, out_dir / "marginals.png",
                   n_per_cluster=args.n_marginal_per_cluster,
                   n_special=args.n_marginal_special)
...
def parse_args():
    ...
    p.add_argument("--scenario", type=str, default="switch",
                   choices=["switch","cascade","switch_cascade",
                            "shared_targets","parallel_sync","early_response"])
    p.add_argument("--n_shared", type=int, default=3,
                   help="Number of shared target genes (shared_targets scenario)")
    p.add_argument("--n_hubs",   type=int, default=3,
                   help="Number of hub genes (parallel_sync scenario)")
    p.add_argument("--n_jobs",   type=int, default=1,
                   help="Parallel simulation jobs (>1 for large G)")
    p.add_argument("--n_marginal_per_cluster", type=int, default=2,
                   help="Genes per cluster to show in marginal distributions")
    p.add_argument("--n_marginal_special", type=int, default=3,
                   help="Special genes (shared/hub) to show in marginals")
    # Optimiser
    ...
    p.add_argument("--skip-umap", action="store_true", default=False)
    return p.parse_args()
