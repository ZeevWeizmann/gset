...
def simulate_multiple_runs(
    ...
    Set n_jobs > 1 for parallel execution (recommended for G > 100).
    ...
    def _run(r):
        ...
        return simulate_dataset(model, scenario, seed=base_seed + r, verbose=verbose)

    if n_jobs == 1 or n_runs == 1:
        return [_run(r) for r in range(n_runs)]

    # Parallel execution
    from multiprocessing import Pool
    if verbose:
        print(f"Running {n_runs} simulations in parallel (n_jobs={n_jobs})...")
    with Pool(processes=min(n_jobs, n_runs)) as pool:
        datasets = pool.map(_run, range(n_runs))
    return datasets
